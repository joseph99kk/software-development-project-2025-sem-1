...
from anaconda3.Lib.idlelib.idle_test.test_config import usercfg
from anaconda3.Lib.threading import settrace_all_threads
...
class ForgotPasswordView(APIView):
    def post(self,request):
        ...
        salt=uuid.uuid4().hex
        token=hashlib.sha512((str(user.id)+user.password+created_at.isoformat()+salt).encode("uft-8")).hexdigest()
        token_obj={"token":token,"created_at":created_at,"expires_at":expires_at,"user_id":user.id}
